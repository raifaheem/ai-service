...
from app.schemas import ChatRequest, UserProfile, HistoryTurn
...
class TestResolveAddonPrompt:
    def test_no_addon(self):
        intent = IntentResult(category="general_health", confidence=0.9, requires_followup=False, detected_entities={}, risk_level="low")
        result = _resolve_addon_prompt(intent, "en")
        assert result is None

    def test_with_addon(self):
        intent = IntentResult(category="symptom_check", confidence=0.9, requires_followup=False, detected_entities={}, risk_level="medium")
        result = _resolve_addon_prompt(intent, "en")
        if intent.addon_name:
            assert result is not None

    def test_with_followup(self):
        intent = IntentResult(category="symptom_check", confidence=0.9, requires_followup=True, detected_entities={}, risk_level="medium")
        result = _resolve_addon_prompt(intent, "en")
        if result:
...
class TestChatEndpoint:
    async def test_chat_success(self, mock_redis, mock_qdrant):
        mock_intent = IntentResult(category="general_health", confidence=0.9, requires_followup=False, detected_entities={}, risk_level="low")
        ...
        with patch("app.services.redis_client._redis", mock_redis), \
             ...
             patch("app.routers.chat.generate_health_answer", new_callable=AsyncMock, return_value="Drink water and rest."), \
    ...
    async def test_chat_off_topic(self, mock_redis, mock_qdrant):
        mock_intent = IntentResult(category="off_topic", confidence=0.9, requires_followup=False, detected_entities={}, risk_level="low")
...
class TestChatStreamEndpoint:
    ...
    async def test_stream_success(self, mock_redis, mock_qdrant):
        ...
        async def mock_stream(*args, **kwargs):
            ...
            yield {"type": "usage", "usage": {"prompt_tokens": 10, "completion_tokens": 5, "total_tokens": 15}, "model": "gpt-4o-mini", "finish_reason": "stop"}
